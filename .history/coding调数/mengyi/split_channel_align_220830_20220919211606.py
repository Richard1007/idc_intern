...
### STEP1: Merge
def merge(file1=channelsum,file2=segpro,merge_label,drop_label)
    channelsum1=file1.loc[file1.Unitschannelmapping>0,:]
    ...
    channelsum2.count()
    segpro1=file2.loc[file2.Units>0,:]
...
segpro3 = merge(channelsum,segpro,merge_label,drop_label)



### STEP2: Melt
def melt(file=channelmix,melt_id_vars,melt_value_vars,melt_var_name,melt_value_name):
    channelmix1=pd.melt(file,id_vars=melt_id_vars,value_vars=melt_value_vars,var_name=melt_var_name,value_name=melt_value_name)
    return channelmix1
...
### STEP3: Merge again
def merge_again(file1=segpro3,file2=channelmix1,merge_label,merge_label)
    segpro4=pd.merge(file1,file2,on=merge_label,how='left')
...
### STEP4:Distribute
def distribute(clist,file=segpro7,chnnl_columns):
